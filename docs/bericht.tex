...
Die hier beschriebene Arbeit behandelt die Lösung der Aufgabe Intellitank. Diese ist Bestandteil des 13. InformatiCup-Wettbewerbs welcher von der Gesellschaft für Informatik e.V. organisiert wird.
Anlässlich des InformatiCup's treten Teams aus verschiedenen Universitäten gegeneinander an um Ihre Lösungen vorzustellen. Die besten vier werden durch eine Jury gekürt und erhalten einen Preis.

\section{Aufgabenstellung}

Das diesjährige Problem, welches es zu lösen gilt wurde 'Intellitank' getauft. Aufgabe ist es ein Modell zu entwerfen, welches es ermöglicht zukünftige Benzinpreis(Superkraftstoff E5)-Entwicklungen an allen Tankstellen Deutschlands mit einer gewissen Signifikanz - aber mindestens einen Monat voraus, vorhersagen zu können. Als Grundlage dieser Arbeit wird von 'Tankerkönig.de' eine Datenbasis zur Verfügung gestellt. Diese beinhaltet die unbereinigten historischen Benzinpreise von rund 15.000 Tankstellen in Deutschland seit 2014. Mithilfe dieser Grundlage ist es möglich eine Korrelationsanalyse durchzuführen. Jedoch ist gefordert, weitere Datenquellen, für ein besseres Ergebnis, mit in das Modell einzubeziehen.\\
Abhängig davon soll eine Anwendung entworfen werden, welche auf Basis des Vorhersage Modells einer gegebenen Route eine optimale Tankstrategie liefert.
Zur Einfachheit wird eine Route definiert durch verschiedene Stopps an Tankstellen und den jeweiligen Zeitpunkt an den man ihn erreicht. Die Anwendung soll also zu diesen Zeitpunkten der jeweiligen Tankstelle einen vorhergesagten Preis festlegen und als Ergebnis zu jeder Tankstelle eine optimale Tankmenge Benzin ermitteln.
