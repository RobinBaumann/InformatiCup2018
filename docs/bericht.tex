...
Bevor wir unser hinreichend gutes Ergebnis mittels der 'Gradient Boosting Machine' auf dem Framework 'h2o' erarbeiteten, wurden verschiedene Verfahren evaluiert um ein Vorhersagemodell zu entwerfen. Unsere Vorgehensweise war allgemein wie folgt:

\begin{description}
	\item[Daten-Präparation]
Die Angleichung und Vorverarbeitung von den Features (auch Feature-Engineering genannt).
	\item[Modell erstellung] 
	Wir erstellten und bewerteten verschiedene Modelle und Algorithmen gegen unsere Domäne.
\end{description}
...
Während der Daten-Präparation zum testen einiger kleiner Modelle, genügte es häufig ein Bruchteil des gesamten Datensatzes zum trainieren zu verwenden und wiederum ein Bruchteil dieses um es zu testen.
...
Anfänglich lagen unsere Daten von unterschiedlichsten Tankstellen zu unterschiedlichen Messzeitpunkten vor. Modelle wie 'ARIMA' benötigen eine gleichbleibende Frequenz der Daten-Zeitpunkten. Also wurden die Daten in eine einheitliche Frequenz abgetastet. Mithilfe von PostGreSQL konnten wir die Daten nun zu stündlichen Messpunkten vorlegen. 
...
Dadurch, dass wir die Benzinpreise stündlich abtasteten, lag es Nahe, dass der vorhergehende Preis nicht nur ähnlich ist, sogar sehr wahrscheinlich derselbe. Denn falls zu einem Zeitpunkt $t_k$ kein neuer Preis vorliegt dann wird der von $t_{k-1}$ übernommen.

\subsection{Modellerstellung}

Durch unsere fälschliche Annahme durch die Partielle Autokorrelation, entworfen wir ein Modell mithilfe der 'Linearen Regression'.
